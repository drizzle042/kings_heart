...
namespace KingsHeart
{
    ...
    App& App::init(const std::string& envFileName)
    {
        ...
        if (App::_app == nullptr)
        {
            ...
            auto mainLogger = spdlog::stdout_color_mt(get_env_t().MAIN_LOGGER);
            ...
            auto commandStorageLogger = spdlog::rotating_logger_mt(
                get_env_t().COMMAND_STORAGE_LOGGER,
                get_env_t().COMMAND_STORAGE_LOGGER_PATH,
                std::stoi(get_env_t().COMMAND_STORAGE_LOGGER_MAX_FILE_SIZE),
                std::stoi(get_env_t().COMMAND_STORAGE_LOGGER_MAX_ALLOWED_FILES));
            commandStorageLogger->set_pattern("%v [%c]");
            commandStorageLogger->set_level(spdlog::level::level_enum::info);

            std::string defaultResponseTemplate{get_env_t().DEFAULT_RESPONSE_TEMPLATE};
            FileRegistry::add_file(defaultResponseTemplate, File{defaultResponseTemplate});

            std::string OTPEmailTemplate{get_env_t().OTP_EMAIL_TEMPLATE};
